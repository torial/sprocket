...
| 2 | ~~`@segments` magic string in the zebra seam~~ | a mode selected by incantation inside another language | **STRUCK 2026-08-08**: BUG-271 (unknown methods) then BUG-2 (deferred bind-list lowering, both Opus 5) landed; procgen emits real `query_segments` / `query_segments_p` methods for every nested proc, no client emits a marker. The seam still *recognizes* `@segments` as a documented hand-written escape hatch, but nothing generates it. The whole table is now struck. |
